...
int sendFrame(unsigned char *buf, int size){
    ...
    if(memcmp(bufReceive[0],FLAG,1) || memcmp(bufReceive[4],FLAG,1)){
           //Terminar
    }

    if(memcmp(bufReceive[1],A,1)){
           //Terminar
    }

    if(memcmp(bufReceive[1] ^ bufReceive[2],bufReceive[3],1)){
           //Terminar
    }
